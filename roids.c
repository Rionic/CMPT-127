...
void ship_draw( const ship_t* ship )
{
  ...
  */int x,y;
	for(x=-1;x<2;x++){
    	for(y=-1;y<2;y++){
        	draw_triangle(ship->x+x+dx,ship->y+y+dy,
        				  ship->x+x+lx,ship->y+y+ly,
        				  ship->x+x+rx,ship->y+y+ry,
        				  shipcolor);
        	if(ship->thrust){
				draw_triangle(ship->x+x-dx,ship->y+y-dy,
							  ship->x+x+lx,ship->y+y+ly,
...
void roid_draw( const roid_t* roid )
{
  ...
  */
	for (int x=-1;x<=1;x++){
    	for (int y=-1;y<=1;y++){
      		draw_rectangle(roid->x+x-roid->width/2.0,
      						roid->y+y-roid->height/2.0,
          					roid->x+x+roid->width/2.0,
          					roid->y+y+roid->height/2,
          					roid->color);
    	}
    }
}

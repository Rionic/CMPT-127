...
uint8_t* copy( const uint8_t array[], 
           ...
           unsigned int rows )
{
    //This code is INCORRECT. Redo!!
   //uint8_t* freshArr = malloc(rows*cols*sizeof(uint8_t));
  //If memory allocation fails, null pointer is returned.
  //if(!freshArr) 
    return NULL;
}
              ...
              unsigned int rows )
{
  for(int i=0; i<rows; i++){
    for(int j=0; j<cols/2; j++){
        int temp=array[j+(i*cols)];
        array[j+(i*cols)]=array[cols-1-j+(i*cols)];
        array[cols-1-j+(i*cols)]=temp;
    }
  }
}
            ...
            unsigned int rows )
{
    for(int i=0; i<cols; i++){
        for(int j=0; j<rows/2; j++){
            int temp=array[i+(j*cols)];
            array[i+(j*cols)]=array[i+(rows-j-1)*cols];
            array[i+(rows-j-1)*cols]=temp;
        }
    }
}
           ...
           unsigned int *y )
{
  for (int i=0; i<rows; i++){
    for (int k=0; k<cols; k++){   
      if ((get_pixel(array,cols,rows,k,i)) == color){
        *x = k;
        *y = i;
        return 1;
      }
            ...
            double scale_factor )
{
  for (int i=0; i<cols*rows; i++){
    float a = round(scale_factor*array[i]);
    if (a > 255){
        array[i] = 255;
    }
    else{
        ...
        unsigned int rows )
{
    ...
    for (int i=0; i<(rows*cols); i++)
        	array[i]=round((array[i]-min)*255.00/(max-min));
}
	       ...
	       unsigned int rows )
{
	...
	uint8_t *new=malloc(cols2*rows2*sizeof(uint8_t));
	if (new!=0){
		for (int x=0;x<cols;x+=2){
			for (int y=0; y<rows ; y+=2){
				float color = round((get_pixel(array,cols,rows,x,y)+
					...
					get_pixel(array,cols,rows,x+1,y))/4.00);
				set_pixel(new,cols2,rows2,x/2,y/2,color);
			}			
		}
		return new;
	}
...
uint8_t* region_copy( const uint8_t array[], 
              ...
              unsigned int bottom )
{
  uint8_t* img = malloc((right-left)*(bottom-top)*sizeof(uint8_t));
  uint8_t color;
  if ((left!=right)&&(top!=bottom)){
    for (int i=left; i<right; i++){
      for (int j=top; j<bottom; j++){
        color=get_pixel(array,cols,rows,i,j);
        set_pixel(img,(right-left),(bottom-top),i-left,j-top,color);
      }
    }
    return img;
  }
